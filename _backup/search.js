...
const marker = L.marker(
  ...
  const bounds = cafesWithCoordinates.map((cafe) => [
    cafe.latitude,
    cafe.longitude
  ]);
  ...
  if (userLocation) {
    ...
    cafesMap.fitBounds(bounds, {
    padding: [40, 40],
    maxZoom: 15
  });
}

// cierra renderMapCafes()
}
...
function renderNearbyCafes() {
  ...
  renderMapCafes(cafesWithDistance);
}
...
function renderCards(cafes, container) {
  ...
  const marker = cafeMarkerMap.get(cafe.name);

  if (!marker || !cafesMap) return;
